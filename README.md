...
| *The online subspace tracker watches its own spectral gap: over 100 ray-traced Munich frames it collapses at frame 22 and reopens at 91, and the gate spends extra refinement passes only while collapsed — 90% lower tracking error than fixed effort. Regenerate with `python -m e2e.main.main_subspace_refine`.* | *Hardware realism is data-driven: 4 interconnect arms at native resolution — legacy placeholder (11-bin mainlobe) vs. ideal/Tessera TSV/Tessera Case3 (1-bin mainlobe each); TSV and Case3 are different, non-overlapping bands, so the lower panel is zoomed to the skirt where they separate from each other. Regenerate with `python -m e2e.main.main_interconnect`.* |
