...
| *The online subspace tracker watches its own spectral gap: over 100 ray-traced Munich frames it collapses at frame 22 and reopens at 91, and the gate spends extra refinement passes only while collapsed — 90% lower tracking error than fixed effort. Regenerate with `python -m e2e.main.main_subspace_refine`.* | *Hardware realism is data-driven: the interconnect block built from HFSS S-parameter measurements vs the legacy placeholder.* |
