...
| *The online subspace tracker watches its own spectral gap and spends extra refinement passes only while it is collapsed. Three arms, so the trade is visible rather than implied: 1 pass/frame is starved everywhere, 60 passes/frame is accurate everywhere, and the gate matches constant effort through the collapse for 37% of its compute — while staying ~15x worse than it outside. The gate buys COMPUTE, not accuracy. Regenerate with `python -m e2e.main.main_subspace_refine --frame-order collapse-window`.* | *Hardware realism is data-driven: 4 interconnect arms at native resolution — legacy placeholder (11-bin mainlobe) vs. ideal/Tessera TSV/Tessera Case3 (1-bin mainlobe each); TSV and Case3 are different, non-overlapping bands, so the lower panel is zoomed to the skirt where they separate from each other. Regenerate with `python -m e2e.main.main_interconnect`.* |
