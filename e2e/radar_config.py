"""
FMCW MIMO radar configuration -- core to the whole simulator, not ML-specific.
...
dataset transforms (`e2e.environment.scatterers`, `e2e.chain.rd_synth`,
`e2e.chain.transforms`).

Nothing here imports torch or numpy -- like `e2e/scenario.py` (its sibling in this
package), this module is intentionally dependency-free (stdlib only) so it can be
built/validated/serialized anywhere, independent of the heavy synthesis code.
"""
...
class RadarConfig:
    ...
    # SNR of every target at every range. See `e2e.chain.link_budget` for the derivation and
