...
The Ka-band TSV arm here loads ``tessera_tsv_s21_public.csv`` -- the DIRECT evaluation
of the public Tessera checkpoint at its own canonical geometry (see
``e2e/data/interconnect/regenerate_tessera_tsv.py``), reproducible from a clean clone.
The legacy ``tessera_tsv_s21.csv`` (``e2e.blocks.TESSERA_INTERCONNECT_CSV``) is NOT
reproducible from the public release (notes/ESTABLISHED_FACTS.md F89/F90) and is kept
only for continuity with older figures -- see the data README. The six 77 GHz
...
#: The Ka-band TSV arm this tutorial actually plots: the public checkpoint's DIRECT
#: evaluation at its own canonical geometry (radius 5/pitch 60/height 100/liner 0.5 um,
#: 300 K, ring3x3), regenerated by `e2e/data/interconnect/regenerate_tessera_tsv.py`.
#: Reproducible from a clean clone plus `python -m e2e.interconnect_surrogate.fetch`.
#: NOT the same as `e2e.blocks.TESSERA_INTERCONNECT_CSV` -- see the module docstring and
#: e2e/data/interconnect/README.md for why that legacy file is kept but not plotted here.
TESSERA_TSV_PUBLIC_CSV = _INTERCONNECT_DATA / "tessera_tsv_s21_public.csv"
...
def main(show=True, band=BAND, n_freqs=N_FREQS):
    ...
    ic = InterconnectBlock(transfer_csv=TESSERA_TSV_PUBLIC_CSV, band_hz=band)
    ...
    if show:
        ...
        print("  from e2e.blocks import InterconnectBlock")
        print("  from e2e.main.main_interconnect import TESSERA_TSV_PUBLIC_CSV")
        print("  ic = InterconnectBlock(transfer_csv=TESSERA_TSV_PUBLIC_CSV,")
...
def _interconnect_arms(all_cases=False):
    ...
    arms = {
        ...
        "tessera_tsv": ("Tessera TSV (Ka-band, 28.5-31.5 GHz, public checkpoint)",
                         InterconnectBlock(transfer_csv=TESSERA_TSV_PUBLIC_CSV, band_hz=BAND)),
    }
    if all_cases:
...
def range_profile_comparison(show=True, n_freqs=N_FREQS):
    ...
    - **Tessera TSV**: the Ka-band (28.5-31.5 GHz) public-checkpoint TSV surrogate
      (`TESSERA_TSV_PUBLIC_CSV`, the DIRECT evaluation -- see the module docstring).
    ...
    """
    freq_tsv, s21_tsv = load_interconnect_transfer(TESSERA_TSV_PUBLIC_CSV)
