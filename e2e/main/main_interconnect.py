...
The Ka-band TSV arm here loads ``e2e.blocks.TESSERA_INTERCONNECT_CSV``
(``tessera_tsv_s21_public.csv``) -- the DIRECT evaluation of the public Tessera
checkpoint at its own canonical geometry (see
``e2e/data/interconnect/regenerate_tessera_tsv.py``), reproducible from a clean clone.
The legacy ``tessera_tsv_s21.csv`` (``e2e.blocks.LEGACY_TESSERA_INTERCONNECT_CSV``) is
NOT reproducible from the public release (notes/ESTABLISHED_FACTS.md F89/F90) and is
kept only for continuity with older figures -- see the data README. The six 77 GHz
automotive HFSS designs are unaffected by any of this and are loaded as before.
...
def main(show=True, band=BAND, n_freqs=N_FREQS):
    """Load the public Tessera TSV interconnect model, show how InterconnectBlock
    applies it, and (when `show`) save a figure. Returns a dict of the computed
    quantities."""
    # 1) The public Tessera checkpoint's direct evaluation: S21(f) over its full sweep.
    ...
    if show:
        ...
        ax1.set_title("Tessera TSV interconnect model S21(f) (public checkpoint)")
...
def _interconnect_arms(all_cases=False):
    ...
    arms = {
        ...
        "tessera_tsv": ("Tessera TSV (Ka-band, 28.5-31.5 GHz, public checkpoint)",
...
def range_profile_comparison(show=True, n_freqs=N_FREQS):
    ...
    - **Tessera TSV**: the Ka-band (28.5-31.5 GHz) public-checkpoint TSV surrogate
      (`TESSERA_INTERCONNECT_CSV`, the DIRECT evaluation -- see the module docstring).
