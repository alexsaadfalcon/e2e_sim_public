...
This is the high-fidelity sibling of `e2e.chain.rd_synth`: instead of evaluating the
...
`complex64 [n_rx, n_chirps, n_samples]` -- so `e2e.chain.transforms`, `e2e.ml.labels`
...
below, no star imports) so every existing `from e2e.environment.rt_gen import ...` /
monkeypatch target keeps working unchanged:

* `e2e.environment.rt_scene_build` -- mesh/asset/scene construction: `RTScene`,
  `build_rt_scene`, the local (unshipped) asset library, `object_local_height_m`, the
  mesh-path/PLY helpers. See that module's docstring for the "Materials" and
  "Ground-rest placement" notes.
* `e2e.environment.rt_signal_chain` -- ray-paths -> ADC signal physics: the CFR->beat
  mapping (`rt_cfr_frame`/`rt_synthesize_adc`/`rt_retrace_reference`), Doppler-validity
...
* `e2e.environment.rt_doppler_study` -- the native-vs-re-trace experiment harness
  (`doppler_error_study`/`format_error_study`) and the `python -m e2e.environment.rt_gen`
  CLI (`main`/`build_arg_parser`), unchanged.

Sionna is imported lazily (inside functions) by all three, so
`import e2e.environment.rt_gen` still works on a machine without Sionna/DrJit -- only
the generation calls need it.
"""
...
# --------------------------------------------------------------------------------
from e2e.environment.rt_scene_build import (  # noqa: F401
    ASSET_LICENSES,
    CAR_ASSET_NAMES,
...
from e2e.environment.rt_signal_chain import (  # noqa: F401
...
from e2e.environment.rt_doppler_study import (  # noqa: F401
    _demo_scenario,
    _rd_peak_bin,
    build_arg_parser,
    doppler_error_study,
