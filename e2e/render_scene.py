...
# `Camera.orientation`/`.look_at()` (see tests/test_render.py's calibration test),
# not guessed.
...
def _build_camera(cam_pos: np.ndarray, look_at: np.ndarray, rt):
    ...
    docs alone) -- see `tests/test_render.py`'s calibration-derived tests.
    """
...
def _build_rt_scene_for_render(scenario, cfg, *, frame_idx: int = 0,
    ...
    `tests/test_render.py::test_build_rt_scene_for_render_*`.
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="python -m e2e.render_scene",
