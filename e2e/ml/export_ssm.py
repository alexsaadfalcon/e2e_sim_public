...
REAL, ray-traced corpus path -- see that module and `e2e/environment/rt_scenes.py`), select the
...
`e2e.environment.rt_scenes.build_rt_tier_scenario(tier, frame_idx=<scene index>, seed=<manifest
...
# Pedestrian footprint (length, width) metres -- mirrors `e2e.environment.rt_scenes._FOOTPRINT_M
...
* Scene determinism: each scene's `Scenario` is a deterministic function of
  ...
  `e2e.environment.rt_scenes.build_rt_tier_scenario`'s "Determinism" docstring section for the
...
* Mesh pools used to build these scenes are a mix of Apache-2.0 (Sionna-bundled
  ...
  `e2e/environment/rt_gen.py`'s `ASSET_LICENSES` table for the exact per-asset provenance if
