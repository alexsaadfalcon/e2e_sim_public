...
# Speed of light, m/s (matches `e2e.radar_config.C_MPS`; duplicated to keep this
...
REFERENCE_PEAK = "peak"
#: Absolute k*T*B*F from `e2e.chain.link_budget`, independent of the cube's contents.
#:
...
def _mimo_tx_factor(cfg, sin_az: torch.Tensor, n_chirps: int) -> torch.Tensor:
    ...
    Mirrors `e2e.chain.rd_synth.synthesize_adc`'s tx_factor EXACTLY (same conventions the
    demux inverts -- see `e2e.chain.transforms.ddma_demux` / `tdm_deinterleave`):
...
class LeakageParams:
    ...
    # -85 dBm (see `e2e.chain.link_budget`):
