...
def _gate_code_diff(outputs):
    """The `max |diff| = N of TOTAL LSB (...)` figure the runner prints on every live
    run -- N, in THIS run's own quantizer LSB units (see `_StoredADCGateBlock`)."""
    note = next(n for n in outputs["_axis_meta"]["notes"] if "live chain vs stored ADC" in n)
    return int(note.split("max |diff| = ")[1].split(" ")[0])


def _strip_chain_flags_meta(files):
    """Rewrite each frame's meta to drop the chain-topology provenance keys
    (`use_rffe`/`use_interconnect`/`use_link_budget`/`quant_bits`), so the frame reads
    exactly as one written before `e2e.ml.chain_generate._ChainFlagsStage` existed --
    the case the live-chain gate's ORIGINAL (generic-guess) wording must still cover
    byte-for-byte. Only the meta string changes; every other array is round-tripped
    unchanged."""
    for f in files:
        with np.load(f, allow_pickle=False) as data:
            arrays = {k: data[k] for k in data.files if k != "meta"}
            meta = json.loads(str(data["meta"].item()))
        for key in ("use_rffe", "use_interconnect", "use_link_budget", "quant_bits"):
            meta.pop(key, None)
...
def test_the_lsb_diff_for_a_12_bit_run_never_exceeds_its_own_code_range(tmp_path, grid,
                                                                        torch_device):
    """The reported unit is THIS run's own quantizer LSB (reviewed 2026-09-23: the
    storage codec's int16 scale used to leak through, e.g. "max |diff| 18456 codes" on
    a screen whose knob said 12-bit -- a 4096-code converter). However mismatched the
    live and stored cubes are, a 12-bit run cannot report more than 2**12 = 4096 LSBs
    apart, because both cubes are clipped to the converter's own code range before
    differencing."""
    manifest = _generate_corpus(tmp_path, grid, torch_device)
    # A full scale wildly off the stored frames' own AGC deliberately maximizes the
    # reported difference.
    outputs = run_pipeline(_state(manifest, bits=12, full_scale=1e-12), n_steps=2)
    diff = _gate_code_diff(outputs)
    assert 0 < diff <= 4096
    assert "of 4096 LSB (12-bit)" in _notes(outputs)
...
def test_an_unrecorded_chain_difference_is_described_not_blamed(tmp_path, grid,
                                                                torch_device):
    """A corpus written BEFORE `_ChainFlagsStage` existed carries none of its keys, so
    the live chain has no way to know the front end was left off at generation. The
    run must say the difference lies in what a frame does not record -- and must not
    claim the operator moved something. Pinned by stripping the keys a real generator
    now always writes (see the sibling test below for the case where it does not)."""
    manifest = _generate_corpus(tmp_path, grid, torch_device, use_rffe=False)
    _strip_chain_flags_meta(sorted(tmp_path.glob("sample_frame_*.npz")))
    outputs = run_pipeline(_state(manifest), n_steps=2)
    notes = _notes(outputs)
...
def test_a_recorded_chain_difference_names_the_specific_setting(tmp_path, grid,
                                                                 torch_device):
    """The sibling of the test above: a corpus generated by TODAY's generator records
    its own chain topology (`e2e.ml.chain_generate._ChainFlagsStage`), so the same
    "front end left off at generation" case is no longer a guess -- the run note must
    name the RF front end specifically, not fall back to the generic wording."""
    manifest = _generate_corpus(tmp_path, grid, torch_device, use_rffe=False)
    outputs = run_pipeline(_state(manifest), n_steps=2)
    notes = _notes(outputs)

    assert _gate_code_diff(outputs) > 0
    assert "RF front end (this run: on vs frames: off)" in notes
    assert "does NOT record" not in notes


def test_a_recorded_bit_depth_difference_names_both_numbers(tmp_path, grid,
                                                             torch_device):
    """The other recorded setting (`quant_bits`): a corpus generated at the default 12
    bits, replayed at 4, must name both numbers rather than guessing."""
    manifest = _generate_corpus(tmp_path, grid, torch_device)
    outputs = run_pipeline(_state(manifest, bits=4), n_steps=2)
    notes = _notes(outputs)

    assert "ADC bit depth (this run: ADC 4-bit vs frames: 12-bit)" in notes
    assert "does NOT record" not in notes
